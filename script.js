...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const corporateDate = new Date('2025-11-21T17:00:00').getTime();
    const countdownElement = document.getElementById("countdown");

    function updateCountdown() {
        const now = new Date().getTime();
        const diff = corporateDate - now;
        if (!countdownElement) return;
        if (diff <= 0) {
            countdownElement.innerHTML = `<div class="corporate-message"><h3>🎉 Корпоратив уже состоялся!</h3><p>Спасибо, что были с нами в этот важный день!</p></div>`;
            return;
        }
    ...
    if (guestName && greetingElement) {
        greetingElement.innerText = `Дорогой(ая), ${decodeURIComponent(guestName)}!`;
    }
